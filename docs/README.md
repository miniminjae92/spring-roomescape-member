...
| 기능           | Method | URL                               | 설명                            |
| ------------ | ------ | --------------------------------- | ----------------------------- |
| 사용자 예약 이력 조회 | `GET`  | `/reservations?name=고래&page=0&size=20` | 사용자가 자신의 이름으로 예약 이력을 페이징 조회한다. |
| 예약 생성        | `POST` | `/reservations`                   | 새로운 예약을 생성한다.                 |
| 예약 일정 변경     | `PUT`  | `/reservations/{id}/schedule`     | 예약의 날짜와 시간 영역을 대체한다.          |
| 예약 취소        | `POST` | `/reservations/{id}/cancellations` | 예약 취소를 요청한다.                  |
...
`name`으로 사용자의 예약 이력을 조회하며, 취소된 예약도 함께 반환한다.
`page`는 0부터 시작하고, `size`는 기본값 20으로 조회한다.
...
예약 전체를 변경하지 않고, 예약의 일정 값 객체인 날짜와 시간을 새 일정으로 대체한다.
